...
From the functionals of surplus consumption ratio reported in \ref{fig:SPCPD}, some basic predictions can be made. We see that when surplus consumption is low, that is in recessionary times, expected returns increases, the intuition is that when times are bad, the risk aversion increases, people wants a higher level of compensation pr. risk unit, that is the price of risk  increases -  note that this must hold true as the risk-free rate is fixed in the model.  
...
\input{Tables/RegressionTableregsNB.tex}
