...
disp([' & Mean consumption growth                & $g$            & $', num2str(0.0134), '$ \\'])
disp([' & Standard deviation of $\Delta c_t$     & $\sigma$         & $', num2str(0.0152),'$ \\'])
disp([' & Standard deviation of $\Delta d_t$     & $\sigma_w$       & $', num2str(0.1256), '$ \\'])
disp([' & Log risk-free rate                     & $r^f$            & $', num2str(0.0109),'$ \\'])
disp([' & Persistence parameter                  & $\phi$           & $', num2str(0.9008),'$ \\'])
disp([' \multicolumn{4}{l}{\textit{Assumed}}                                   \\'])
disp([' & Coefficient of Risk Aversion           & $\gamma$         & $', num2str(2.0000),'$ \\'])
disp([' & Correlation dividends/consumption      & $\rho$           & $', num2str(0.2000),'$ \\'])
disp(['\multicolumn{4}{l}{\textit{Implied}}                                    \\'])
disp([' & Subjective discount factor             & $\delta$         & $', num2str(0.9156),'$ \\'])
disp(['& Steady-state surplus consumption ratio & $\Bar{S}$        & $', num2str(0.0666),'$ \\'])
disp([' & Maximum surplus consumption ratio      & $S_{\text{max}}$ & $', num2str(0.1096),'$ \\ \bottomrule'])
disp('\end{tabular}')
...
disp(['$\mathbb{E}\left[r_t- r^f_t\right]$& $', num2str(0.0373),'$           & $', num2str(0.0927),'$          \\'])
disp(['$\sigma\left(r_t - r^f_t  \right)$ & $', num2str(0.0962),'$           & $', num2str(0.1670),'$          \\'])
disp(['$\mathbb{E}\left[r_t- r^f_t\right] / \sigma\left(r_t - r^f_t,\right)$ & $', num2str(0.3877),'$ & $', num2str(0.5548),'$  \\ \bottomrule'])
disp(['\end{tabular}'])
disp(['\end{table}'])
diary off
